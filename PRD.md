...
## Epic 13 — Stack-Aware Implementer Variants [MUST] [DONE 2026-04-30]
...
- **Change**: Add Spring Boot detection by reading `pom.xml` (`<artifactId>spring-boot-starter*</artifactId>`) and `build.gradle` / `build.gradle.kts` (`org.springframework.boot`). Add ASP.NET Core detection by scanning discovered `*.csproj` files deterministically until a `Microsoft.AspNetCore.*` package reference is found. Export new helpers `isBackendFramework(framework)`, `isFullstackJsFramework(framework)` in [src/constants/frameworks.ts](src/constants/frameworks.ts) (Next.js / Nuxt / Remix / SvelteKit are fullstack; NestJS / Express / Fastify / Hono / FastAPI / Django / Flask / Spring Boot / ASP.NET Core are backend). Keep existing exports intact.
...
- **Change**: Remove the `reactTsSenior` row from `askAgentSelection` and never introduce `*-senior` rows. Gate `ui-designer` on `isFrontendFramework(stack.framework)` after the user confirms the stack — hidden (not merely unchecked) when absent. Add `askImplementerVariant(stack)` returning `ImplementerVariant`: a single-select question whose default is `getApplicableImplementerVariantForStack(stack)` and whose choices are the full enum plus inline labels describing each variant. In `runPromptFlow`, call `askAgentSelection({ isFrontend })` first, prompt for the variant only when `implementer` remains enabled, and thread the selected or derived value into `agents.implementerVariant`. Drop the `isReactTs` parameter from `askAgentSelection`.
...
- **Change**: After generated files and the manifest are written successfully, scan the project for `.claude/agents/react-ts-senior.md` and `.codex/skills/react-ts-senior/SKILL.md`. If found, route them through the Epic 7 safe-delete confirmation path: show a one-line message (`Removing stale file replaced by implementer variant: <path>`) and a `y/N` prompt (skipped when `--yes` is passed). Never hard-delete without confirmation; back up via `backupExistingFiles` only after deletion is confirmed or prompts are suppressed. Log skipped removals as warnings without creating backup artifacts.
