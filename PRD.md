...
- **Change**:
  - `AGENTS.md.ejs`: add a concise **Cross-model handoff setup** section after the "Stack-aware writer/reviewer defaults" block naming the mechanisms from §1.7.2 (Codex plugin — primary; subprocess fallbacks only with explicit operator approval) and the one-time install commands (`/plugin marketplace add openai/codex-plugin-cc`, `/plugin install codex@openai-codex`, `/codex:setup`). Keep it under 15 rendered lines; do not duplicate §1.7.2 prose.
...
- 9-state checklist is always rendered: `default`, `hover`, `focus` (`focus-visible:` ring with ≥3:1 contrast per WCAG 2.2 SC 1.4.11), `active`, `disabled` (`aria-disabled` + `disabled:` variant; never communicate disabled by color alone), `loading` (spinner/skeleton with `aria-busy="true"` + accessible label; preserve layout to keep CLS = 0), `empty` (actionable empty state with primary CTA), `error` (`aria-invalid="true"` + `aria-describedby`), `success` (paired with text, not color alone, per WCAG SC 1.4.1).
- 5-a11y considerations always rendered with WCAG 2.2 SC numbers: keyboard navigation (SC 2.1.1, 2.1.2), screen reader labels (SC 2.5.3), focus order (SC 2.4.3, 2.4.11 — focus must not be obscured by sticky headers/toasts), color contrast (text ≥4.5:1, large text ≥3:1, UI components and focus indicators ≥3:1), motion sensitivity (`prefers-reduced-motion` + Tailwind `motion-reduce:`/`motion-safe:` + iOS Reduce Motion + Android no-heavy-parallax). New WCAG 2.2 AA criteria explicitly cited: 2.4.11 Focus Not Obscured (Minimum), 2.5.7 Dragging Movements, 2.5.8 Target Size (Minimum) ≥24×24 CSS px; new A criteria: 3.2.6 Consistent Help, 3.3.7 Redundant Entry.
...
- RTL + i18n block always rendered: prefer logical properties (`ms-*`/`me-*` for margin-inline-start/end, `ps-*`/`pe-*` for padding-inline-start/end, `start-*`/`end-*` for inset-inline-start/end) and the `ltr:`/`rtl:` Tailwind variants so layouts mirror automatically under `dir="rtl"`; note that Reka UI / Radix UI primitives ship native RTL direction support.
...
### E18.T3 — Mobile + Flutter + vanilla fixtures [§18] — M

- **Files**: new `tests/fixtures/frontend-vanilla/{package.json,index.html}`, `tests/fixtures/mobile-rn/{package.json}` (with `react-native`), `tests/fixtures/mobile-expo/{package.json,app.json}` (with `expo`), `tests/fixtures/mobile-flutter/{pubspec.yaml,lib/main.dart}` (with `flutter` SDK constraint). New detector helper `detectFlutter` in [src/detector/detect-framework.ts](src/detector/detect-framework.ts) reading `pubspec.yaml` for `sdk: flutter` (Epic 13's E13.T1 mentions detector helpers but Flutter detection currently isn't covered).
- **Change**: Minimal manifests per fixture (≤30 LOC each — no source code beyond what the detector needs). The Flutter fixture proves Flutter detection. Wire each fixture into the snapshot suite from E18.T6. Update `isFrontendFramework(framework)` from Epic 13's E13.T1 to include `'flutter'` so the `ui-designer` row gates correctly for Flutter projects.
- **Done when**: each fixture loads, `detectStack` returns the expected `framework` + `language` pair, and `detectCssEngine` returns the expected value (e.g., `'unknown'` for Flutter and the mobile-RN/Expo fixtures unless they actually depend on NativeWind/Tailwind).
